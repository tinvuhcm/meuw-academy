...
DEFAULT_TEST_DIR="${REPO_ROOT}/process/general-plans/reports/visuals/tech-graph-test-output"
TEST_DIR="${TECH_GRAPH_TEST_OUTPUT_DIR:-$DEFAULT_TEST_DIR}"
...
PY
        ...
        if python3 "${SKILL_DIR}/scripts/generate-from-template.py" "$TEMPLATE_TYPE" "$SVG_FILE" "$(cat "$FIXTURE")" > /dev/null 2>&1 \
            ...
            if command -v rsvg-convert &> /dev/null \
                ...
                echo -e "${GREEN}✓ Pass${NC} (SVG-only mode)"
            fi
            PASSED=$((PASSED + 1))
        else
            echo -e "${RED}✗ Fail${NC}"
